...
SLIDES_WIDTH=1250
SLIDES_HEIGHT=504
SLIDES_MOBILE_WIDTH=750
SLIDES_MOBILE_HEIGHT=600
EAN_WIDTH=400
EAN_HEIGHT=600
...
check_file() {
  ...
  if [ "$max_width" -gt 0 ] && [ "$dims" != "?" ]; then
    ...
    if [ "$width" -gt "$max_width" ] || [ "$height" -gt "$max_height" ]; then
      if [ -n "$reason" ]; then
        reason="${reason}, piksel > ${max_width}x${max_height}"
      else
  ...
  for file in "$slides_dir"/*; do
    ...
    case "$(basename "$file")" in
      *m.jpg|*m.jpeg|*m.png|*m.webp)
        check_file "$file" "$SLIDES_MOBILE_SIZE_KB" "$SLIDES_MOBILE_WIDTH" "$SLIDES_MOBILE_HEIGHT"
        ;;
      *)
        check_file "$file" "$SLIDES_SIZE_KB" "$SLIDES_WIDTH" "$SLIDES_HEIGHT"
        ;;
    esac
  done
}
